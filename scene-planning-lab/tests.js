(async function () {
  ...
  try {
    ...
    test("A new plan opens on the complete worked example with three student scenes available", function () { var state = core.createState(data); return state.currentStep === 1 && state.scenes.length === 3; });
    ...
    test("An older saved revision opens on the revised worked example", function () { var state = core.normalizeState(data, { contentRevision: "older-story", currentStep: 4 }); return state.currentStep === 1 && state.contentRevision === data.revision; });
    ...
    summary.textContent = failures ? failures + " test(s) failed." : "All 11 tests passed.";
  } catch (error) {
